...
int main()
{
    ...
    char rootName[MAX_NAME_LEN];  // 왕조의 시조 이름
    ...
    root->depth = 0;                                   // 깊이
    char son[MAX_NAME_LEN], father[MAX_NAME_LEN];       // 관계 정보 저장용
    int i = 1;                                          // 깊이 초기화
    Node *fatherNode;
    Node* sonNode;
    while (fscanf(fp, "%s %s", son, father) == 2)        // 관계 한 줄씩 읽기
    {
        // 아버지 노드가 없으면 생성 후 리스트에 추가
        if (!(fatherNode = findNode(nodeList, nodeCount, father)))
        {
            fatherNode = createNode(father);
            nodeList[nodeCount++] = fatherNode;// 동적메모리 해제를 위해
            fatherNode ->depth = i++;
        }
        // 아들 노드가 없으면 생성 후 리스트에 추가
        if (!(sonNode = findNode(nodeList, nodeCount, son)))
        {
